...
def user_delete(id):
    ...
    if not user:
        ...
        try:
            ...
            db.session.rollback()
            flash('Nie można usunąć Użytkownika, ponieważ jest używany jako klucz obcy w innych tabelach.', 'error')
        return redirect(url_for('auth_bp.auth'))
...
def role_delete(id):
    ...
    if not rola:
        ...
        try:
            ...
            db.session.rollback()
            flash('Nie można usunąć Roli, ponieważ jest używana jako klucz obcy w innych tabelach.', 'error')
        return redirect(url_for('auth_bp.roles'))
