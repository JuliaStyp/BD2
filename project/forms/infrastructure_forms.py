...
class LocationForm(Form):
    szerokosc_geo = FloatField(
        "Szerokość geograficzna", validators=[validators.InputRequired()]
    )
    dlugosc_geo = FloatField(
        "Długość geograficzna", validators=[validators.InputRequired()]
    )
    nazwa_miejsca = StringField(
        "Nazwa miejsca", validators=[validators.Optional(), validators.Length(max=32)]
    )

    def validate_szerokosc_geo(form, field):
        if field.data is not None and not (-90 <= field.data <= 90):
            raise ValidationError(
                "Szerokość geograficzna musi być w zakresie -90 do 90 stopni."
            )

    def validate_dlugosc_geo(form, field):
        if field.data is not None and not (-180 <= field.data <= 180):
            raise ValidationError(
                "Długość geograficzna musi być w zakresie -180 do 180 stopni."
            )


class ObjectForm(Form):
    typ = StringField(
        "Typ", validators=[validators.InputRequired(), validators.Length(max=32)]
    )
...
class StatusForm(Form):
    status = StringField(
        "Status", validators=[validators.InputRequired(), validators.Length(max=32)]
    )


class ElementForm(Form):
    typ = IntegerField("Id typu", validators=[validators.InputRequired()])
    lokalizacja = IntegerField(
        "Id lokalizacji", validators=[validators.InputRequired()]
    )
    ...
    opis = TextAreaField(
        ...
        validators=[
            validators.Length(
                min=0, max=1024, message="Opis może zawierać maksymalnie 1024 znaków"
            ),
            validators.Optional(),
        ],
    )
