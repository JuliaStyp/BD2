from wtforms import Form, BooleanField, StringField, PasswordField, IntegerField, TextAreaField,\
                    DateField, DecimalField, EmailField
...
class ServiceForm(Form):
    name = TextAreaField("Nazwa", 
                         validators=[Length(min=0, max=128,   message="Nazwa może mieć maksymalnie 128 znaków"),
                                     InputRequired()])

    phone_number = TextAreaField("Nr telefonu", 
                                validators=[Length(min=9, max=32,   message="Nr telefonu musi mieć przynajmniej 9 znaków."),
                                            InputRequired()])
    mail = EmailField("Adres e-mail", validators=[Length(min=0, max=128,   message="E-mail może mieć maksymalnie 128 znaków"),
                                            InputRequired(),])
